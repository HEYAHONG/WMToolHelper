...
GUIDialog::GUIDialog( wxWindow* parent, wxWindowID id, const wxString& title, const wxPoint& pos, const wxSize& size, long style ) : wxDialog( parent, id, title, pos, size, style )
{
	...
	m_filePicker_firmware = new wxFilePickerCtrl( m_panel_settings, wxID_ANY, wxEmptyString, wxT("选择固件"), wxT("*.fls"), wxDefaultPosition, wxDefaultSize, wxFLP_DEFAULT_STYLE|wxFLP_FILE_MUST_EXIST|wxFLP_OPEN|wxFLP_SMALL );
